$(document).ready(function () {
    ...
    $("#play_btn").click(function() {
            ...
            $.ajax({
                    ...
                    .done( function(results) {
                                        console.log("##TP_CGI_URL##?action=CHEAT_play_action&user_id=##TC_USER_ID##&game_id=##TC_GAME_ID##&kind="+kind+"&cards="+cards);
                                        var response = $(results);
                                        deck = response;
                                        console.log(deck);
                                        console.log(response);
                                        turn=0;
                                        turn_no=results["turn_no"];
                                        });        
                    });
    ...
    $("#cheat_btn").click(function() {
            ...
            $.ajax({
                    ...
                    .done( function(results) {
                                        console.log("##TP_CGI_URL##?action=CHEAT_play_cards&user_id=##TC_USER_ID##&game_id=##TC_GAME_ID##");
                                        var response = $(results);
                                        deck = response;
                                        console.log(deck);
                                        console.log(response);
    ...
    setTimeout(function CheckTurn() {
                              $.ajax({
                                      ...
                                      dataType: "json"
                                                                ...
                                                                console.error("ehiohi");
                                                                turn_no=results["turn_no"];
                                                                console.log("check_status");
                                                                if ( results["cur_player_id"]==PERSONAL_USER_ID) 
                                                                    {
                                                                      turn=1;
                                                                  ...
                                                                  console.log("need to update all the cards and visuals")
                                                                  drawGameBoard(results);

                                                                }
                                                              });                   
                                      setTimeout( CheckTurn , 4000);
                                  }, 1 );

    // setTimeout(function Timeout(){
    //                               if (##TC_TURN##==PERSONAL_USER_ID)
    //                               {
    //                                 console.log("timeout");
    //                               } 
    ...
    $(".selection").click(function() {
                                      var kind = $(this).attr("id").match(/[0-9]+/)[0];
                                      console.log(kind);
                                      });
